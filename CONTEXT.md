...
**Candidate**:
A proposed correspondence between an imported feature and a base feature, with evidence and separate eligibility for copying tags, connecting networks, and removing an equivalent imported way. A candidate does not by itself select an action.
_Avoid_: Confirmed match

**Match evidence**:
The locations, measured differences, attributes, and connection context used to assess a proposed correspondence. Proximity is evidence of a possible match, not proof of feature identity or that a matching action is safe.
...
Contradictory explicit classifications on a proposed pair, such as a base cafe and an imported school. The conflict prevents matching actions for that pair, independently of which attributes were selected for copying.
...
**Selected target**:
The base counterpart for which at least one matching action is scheduled. Choosing a target can select copying or connection actions, while removal requires its own explicit choice; scheduling an action for another counterpart replaces the prior target.
...
**Network attachment**:
The API term for connecting the imported network to the base network. Selecting this action is independent of selecting Copy tags or Remove imported way.
...
**Geometry removal**:
The explicit removal of a redundant imported way after establishing an equivalent retained base counterpart and preserving required connections. It is distinct from copying tags and connecting the network.

**Remove imported way**:
The user-facing choice to schedule geometry removal for one reviewed imported way. Its remaining attributes leave with it; copying selected attributes to the base is a separate choice.

**Orphan-node cleanup**:
The removal of untagged imported points left without any way or relation references by an explicitly removed way. Tagged, still-referenced, and authoritative base points are retained.
...
**Automatic action**:
A copying or connection action scheduled by the configured rules without an individual selection. A saved choice can turn it off; geometry removal is never automatic, and automatic does not mean already applied.
_Avoid_: Applied automatically, completed match

**Applied action**:
A matching action that changed the result when matching was applied, before intersection creation. Copying one or more tag values counts as one tag-copy action for an imported feature; changing one or more imported way references for a matched node counts as one network-connection action. Removing one redundant imported way counts as one way-removal action, with newly orphaned points counted separately. An eligible, scheduled, or already-satisfied action is not an applied change. Later intersection work can further connect or remap junctions.
