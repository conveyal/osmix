...
maplibregl.addProtocol("@osmix/vector", async (params, abort) => {
  ...
  const data = await remote.getVectorTile(decodeURIComponent(id), [+x, +y, +z] as Tile);
...
async function init() {
  ...
  // Restore from IDB
  const [pbf, name] = await Promise.all([idb.get(IDB_KEY.PBF), idb.get(IDB_KEY.NAME)]);
  if (pbf && name) {
...
function initRouteMap(info: OsmRemoteDataset) {
  ...
  routeMap.on("load", () => {
    ...
    routeMap.addLayer({
      ...
      type: "line",
      paint: {
        "line-color": ["case", ["has", "color"], ["to-color", ["get", "color"]], "white"],
        "line-width": ["interpolate", ["linear"], ["zoom"], 12, 0.5, 18, 10],
      },
    });
    routeMap.on("click", (e) => handleRouteClick(info, [e.lngLat.lng, e.lngLat.lat]));
...
async function handleRouteClick(dataset: OsmRemoteDataset, pt: [number, number]) {
  ...
  if (!snap) return updateRouteTable([...rows, ["Error", "No road found within 500m."]]);
  ...
  if (!routeState.origin) {
    ...
    const res = await dataset.route(routeState.origin.nodeIndex, routeState.dest.nodeIndex, {
      includeStats: true,
