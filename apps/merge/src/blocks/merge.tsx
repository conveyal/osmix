	...
	return (
		<>
			...
			<Step step="inspect-base-osm" title="INSPECT BASE OSM">
				...
				<p>
					Duplicates are features that share an ID or occupy the same geometry.
					We prefer entities with newer version metadata; if that information is
					missing we keep the feature with more tags.
				</p>
				<p>
					When a duplicate is detected we draft a changeset entry that removes
					the extra copy. Review those proposals in the next step before
					applying them.
				</p>
				<Card>
			...
			<Step step="inspect-patch-osm" title="INSPECT PATCH OSM">
				<p>
					Generate a changeset that removes duplicate entities from the patch
					file before it is merged into the base data.
				</p>
			...
			<Step step="create-intersections" title="CREATE INTERSECTIONS">
				<div className="flex flex-col space-y-2">
					<p>
						Scan new ways for crossings with existing ways and flag the segments
						that should share intersection nodes based on their tags.
					</p>
					<p>
						We quickly search for nearby ways and keep only those whose tags
						allow an intersection: both must be linear, share the same `layer`
						value if present, include a `highway` tag, and avoid bridge or
						tunnel tags.
					</p>
					<p>
