...
let dataset = await remote.fromPbf(Readable.toWeb(createReadStream(pbfPath)) as ReadableStream, {
  id: filename,
});
...
app.get("/tiles/:z/:x/:y", async (c) => {
  try {
    ...
    const tile = await remote.getWorker().getShortbreadTile(dataset.id, [+x, +y, +z]);
    console.timeEnd(url);
