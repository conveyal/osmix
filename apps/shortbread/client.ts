...
function handleDragOver(e: DragEvent) {
  ...
  if (fileItems.length > 0) {
    ...
    if (fileItems.length === 1) {
      document.body.style.boxShadow = "inset 0 0 1px 4px rgba(from #3b82f6 r g b / 0.5)";
    } else {
      document.body.style.boxShadow = "inset 0 0 1px 4px rgba(from #ef4444 r g b / 0.5)";
    }
  }
}
...
function updateLayerControl(map: maplibregl.Map, $layerControl: HTMLElement) {
  ...
  for (const layerId of layers) {
    ...
    if (!layer) continue;
    const sourceLayer = "source-layer" in layer ? (layer["source-layer"] as string) : "base";
  ...
  $layerControl.querySelectorAll("input").forEach((input) => {
    input.addEventListener("change", (e) => {
      ...
      map.setLayoutProperty(target.name, "visibility", target.checked ? "visible" : "none");
    });
  });
}

function addMapControl(map: maplibregl.Map, id: string, placement: ControlPosition) {
