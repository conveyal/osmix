...
function App() {
  ...
  return (
    <div>
      ...
      <div>
        <input ref={fileInputRef} type="file" accept=".pbf,.osm.pbf" onChange={handleFileSelect} />
        {selectedFile && currentStatus.status !== "running" && (
          <button type="button" onClick={() => handleRunBenchmark(selectedFile)}>
            Run Again
          </button>
        )}
      </div>
      ...
      {currentStatus?.status === "running" && (
        <div>
          ...
          >
            <pre style={{ fontWeight: "bold" }}>{currentStatus.currentMetric}</pre>
            <pre>
              {log
                .slice(0, -1)
                .toReversed()
...
export function BenchmarkResultsView({ results }: { results: BenchmarkResults[] }) {
      ...
      <table>
        ...
        <tbody>
          {osmixResults.benchmarks.map((osmixMetric, i) => {
            ...
            if (osmixMetric.time === null || duckdbMetric == null || duckdbMetric.time === null) {
              return (
              ...
              <tr key={osmixMetric.type}>
                <td>
                  <div className="tooltip">
                    {osmixMetric.type}
                    <span className="tooltiptext">{metricDescriptions[osmixMetric.type]}</span>
                  </div>
                </td>
                <td>{osmixMetric.time?.toFixed(2) ?? "—"} ms</td>
                <td>{osmixMetric.result ?? "—"}</td>
        ...
        <tbody>
          <tr>
            <td style={{ width: "50%", height: "500px" }}>
              <MapView bbox={osmixResults.bbox} geojson={osmixResults.geojson} color="red" />
            </td>
            <td style={{ width: "50%", height: "500px" }}>
              <MapView bbox={duckdbResults.bbox} geojson={duckdbResults.geojson} color="yellow" />
            </td>
          </tr>
        </tbody>
      </table>
