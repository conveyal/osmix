...
import type { GeoBbox2D, OsmNode, OsmTags, OsmWay } from "@osmix/types";
...
export class DuckDBBenchWorker {
    ...
    for (let i = 0; i < wayResult.numRows; i++) {
      ...
      if (row.refs) {
        try {
          ...
          } else if (typeof row.refs === "object" && Symbol.iterator in row.refs) {
  ...
  async getGeoJSON(): Promise<GeoJSON.FeatureCollection> {
    ...
    const features: GeoJSON.Feature<GeoJSON.Point | GeoJSON.LineString, OsmTags>[] = [];
