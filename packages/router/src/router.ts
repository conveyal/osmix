...
export class Router {
  ...
  constructor(osm: Osm, graph: RoutingGraph, options: Partial<RouteOptions> = {}) {
    this.osm = osm;
    this.graph = graph;
    this.defaults = {
  ...
  route(
    ...
    if (fromNodeIndex === toNodeIndex) return [{ nodeIndex: fromNodeIndex, cost: 0 }];

    const algorithm = routingAlgorithms[options.algorithm ?? this.defaults.algorithm];
  ...
  buildResult(path: PathSegment[], options: Partial<RouteOptions> = {}): RouteResult {
    ...
    const includePathInfo = options.includePathInfo ?? this.defaults.includePathInfo;
    ...
    for (const seg of path) {
      if (seg.wayIndex !== undefined && seg.previousNodeIndex !== undefined) {
        ...
        const edge = edges.find(
          (e) => e.targetNodeIndex === seg.nodeIndex && e.wayIndex === seg.wayIndex,
        );
        if (edge) {
  ...
  getRoutePathInfo(path: PathSegment[]): RoutePathInfo {
    ...
    for (const { wayIndex, transitionNodeIndex, distance, time } of edgeSequence) {
