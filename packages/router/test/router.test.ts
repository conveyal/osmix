...
describe("Router with Monaco PBF", () => {
  ...
  it("should find routes between points in Monaco", () => {
    ...
    const from = graph.findNearestRoutableNode(monacoOsm, [7.4229093, 43.7371175], 500);
    const to = graph.findNearestRoutableNode(monacoOsm, [7.4259193, 43.7377731], 500);
  ...
  it("should find routes using different algorithms in Monaco", () => {
    ...
    const paths = [dijkstraPath, astarPath, bidirectionalPath].filter((p) => p !== null);
    expect(paths.length).toBe(3);
  ...
  it("should include both stats and path info when both options are true", () => {
    ...
    const segmentDistanceSum = result.segments!.reduce((sum, seg) => sum + seg.distance, 0);
...
describe("RoutingGraph Serialization", () => {
  it("should serialize and reconstruct graph correctly", () => {
    ...
    const isBuffer = (b: unknown) => b instanceof ArrayBuffer || b instanceof SharedArrayBuffer;
    ...
    for (let i = 0; i < osm.nodes.size; i++) {
      ...
      for (let j = 0; j < origEdges.length; j++) {
        expect(reconEdges[j]!.targetNodeIndex).toBe(origEdges[j]!.targetNodeIndex);
    ...
    expect(reconstructedGraph.isIntersection(0)).toBe(originalGraph.isIntersection(0));
    expect(reconstructedGraph.isIntersection(1)).toBe(originalGraph.isIntersection(1));
  });
