 ...
 */
function createGraph(adjacencyList: Map<number, GraphEdge[]>): (nodeId: number) => GraphEdge[] {
 ...
 */
function mockEdge(targetNodeIndex: number, wayIndex: number, distance: number): GraphEdge {
  return {
...
describe("bidirectional", () => {
  ...
  it("should track way indexes in combined path", () => {
    ...
    const wayIndexes = path?.filter((p) => p.wayIndex !== undefined).map((p) => p.wayIndex);
