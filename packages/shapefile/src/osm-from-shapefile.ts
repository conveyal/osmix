...
export function* startCreateOsmFromShapefile(
  ...
  for (const feature of geojson.features) {
    ...
    if (geomType === "Point") {
      ...
      const coords = (normalizedFeature.geometry as GeoJSON.MultiPoint).coordinates;
      ...
      const coords = (normalizedFeature.geometry as GeoJSON.MultiLineString).coordinates;
...
function processPolygonRings(
  ...
  const outerWayId = createRelation ? getNextWayId() : (featureId ?? getNextWayId());
  osm.ways.addWay({
    id: outerWayId,
    refs: outerNodeRefs,
