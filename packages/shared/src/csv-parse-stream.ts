...
export interface CsvParseStreamOptions {
  ...
  mapValues?: (args: { header: string | undefined; index: number; value: string }) => string;
}

const defaults: Required<
  Pick<
    CsvParseStreamOptions,
    "separator" | "quote" | "escape" | "skipFirstRow" | "skipComments" | "strict"
  >
> = {
  separator: ",",
  quote: '"',
      ...
      transform(chunk, controller) {
        const emitRow = (cells: string[]) => {
          ...
          if (options.skipComments) {
            const commentPrefix =
              typeof options.skipComments === "string" ? options.skipComments : "#";
            const firstCell = cells[0] ?? "";
            if (firstCell.startsWith(commentPrefix)) return;
          }

          if (headers === null || (lineNumber === 1 && options.skipFirstRow)) {
            headers = cells.map((header, index) => options.mapHeaders({ header, index }));
            return;
          }

          if (headers === false) {
            const out = Object.fromEntries(cells.map((value, index) => [index, value]));
