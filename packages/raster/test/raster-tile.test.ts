...
describe("@osmix/raster: OsmixRasterTile", () => {
  ...
  it("fills polygons using scanline algorithm", () => {
    ...
    expect(Array.from(tile.imageData.slice(outsideIdx, outsideIdx + 4))).toEqual([0, 0, 0, 0]);
  });
  ...
  it("fills polygons with holes correctly", () => {
    ...
    expect(Array.from(tile.imageData.slice(insideOuterIdx, insideOuterIdx + 4))).toEqual(
      Array.from(DEFAULT_AREA_COLOR),
    );
  ...
  it("handles polygon with hole correctly (winding order)", () => {
    ...
    expect(Array.from(tile.imageData.slice(insideOuter, insideOuter + 4))).toEqual(
      Array.from(DEFAULT_AREA_COLOR),
    );

    // Check that pixels inside inner (hole) are NOT filled
    const insideHole = tile.getIndex([20, 20]);
    const holeColor = Array.from(tile.imageData.slice(insideHole, insideHole + 4));
  ...
  describe("edge boundary handling", () => {
    it("excludes left boundary pixels when polygon crosses left edge", () => {
      ...
      expect(Array.from(tile.imageData.slice(insideIdx, insideIdx + 4))).toEqual(
        Array.from(DEFAULT_AREA_COLOR),
      );
    });
    ...
    it("excludes top boundary pixels when polygon crosses top edge", () => {
      ...
      const topBoundaryColor = Array.from(tile.imageData.slice(topBoundaryIdx, topBoundaryIdx + 4));
    ...
    it("excludes corner boundary pixels when polygon crosses multiple edges", () => {
      ...
      const cornerColor = Array.from(tile.imageData.slice(cornerIdx, cornerIdx + 4));
    ...
    it("ensures adjacent tiles do not create duplicate edge pixels", () => {
      ...
      expect(Array.from(tile1.imageData.slice(tile1InsideIdx, tile1InsideIdx + 4))).toEqual(
        Array.from(DEFAULT_AREA_COLOR),
      );

      const tile2InsideIdx = tile2.getIndex([1, 100]);
      expect(Array.from(tile2.imageData.slice(tile2InsideIdx, tile2InsideIdx + 4))).toEqual(
        Array.from(DEFAULT_AREA_COLOR),
      );
    });
  });
  ...
  describe("line drawing edge cases", () => {
    it("drawLine does not draw pixels outside tile bounds", () => {
      ...
      expect(Array.from(tile.imageData.slice(leftBoundaryIdx, leftBoundaryIdx + 4))).toEqual(
      ...
      expect(Array.from(tile.imageData.slice(rightBoundaryIdx, rightBoundaryIdx + 4))).toEqual(
        Array.from(DEFAULT_LINE_COLOR),
      );
    ...
    it("drawLine handles horizontal lines at boundaries correctly", () => {
      ...
      expect(Array.from(tile.imageData.slice(topBoundaryIdx, topBoundaryIdx + 4))).toEqual(
        Array.from(DEFAULT_LINE_COLOR),
      );
      ...
      expect(Array.from(tile.imageData.slice(bottomBoundaryIdx, bottomBoundaryIdx + 4))).toEqual(
        Array.from(DEFAULT_LINE_COLOR),
      );
    });
  });
  ...
  describe.each(tiles)(
    ...
    (x: number, y: number, z: number) => {
      ...
      it("draws entities that fit in a single pixel", () => {
        ...
        const offsetMin = tile.tilePxToLonLat([centerPx[0] - 0.25, centerPx[1] + 0.25]);
        const offsetMax = tile.tilePxToLonLat([centerPx[0] + 0.25, centerPx[1] - 0.25]);
        const bbox: GeoBbox2D = [offsetMin[0], offsetMin[1], offsetMax[0], offsetMax[1]];
      ...
      it("uses vertical extent when width collapses to zero", () => {
        ...
        const centerOffset = tile.tilePxToLonLat([centerPixel[0] + 0.5, centerPixel[1]]);
