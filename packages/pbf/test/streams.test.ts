...
describe("transform streams", () => {
  it("requires the header to be written before data blocks", async () => {
    ...
    await expect(
      input.pipeThrough(new OsmBlocksToPbfBytesTransformStream()).pipeTo(new WritableStream()),
  ...
  it("parses streamed bytes back into header and primitive blocks", async () => {
    ...
    if (!block.primitivegroup[0]) throw new Error("block.primitivegroup[0] is undefined");
