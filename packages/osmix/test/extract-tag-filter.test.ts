...
describe("extract-tag-filter", () => {
	...
	test("tagRuleMatches any value when value omitted", () => {
		expect(tagRuleMatches({ highway: "primary" }, { key: "highway" })).toBe(
			true,
		)
		expect(tagRuleMatches({ highway: "primary" }, { key: "railway" })).toBe(
			false,
		)
	})
	...
	test("entityMatchesTagRules multiple rules use OR", () => {
		...
		expect(entityMatchesTagRules({ highway: "road" }, rules)).toBe(true)
		expect(entityMatchesTagRules({ public_transport: "platform" }, rules)).toBe(
			true,
		)
		expect(entityMatchesTagRules({ building: "yes" }, rules)).toBe(false)
	})
	...
	test("CONVEYAL_EXTRACT_TAG_FILTERS way regression", () => {
		...
		expect(
			wayMatchesExtractTagRules(
				{ id: 1, refs: [], tags: { highway: "primary" } },
				rules,
			),
		).toBe(true)
		expect(
			wayMatchesExtractTagRules(
				{ id: 2, refs: [], tags: { public_transport: "platform" } },
				rules,
			),
		).toBe(true)
		expect(
			wayMatchesExtractTagRules(
				{ id: 3, refs: [], tags: { railway: "platform" } },
				rules,
			),
		).toBe(true)
		expect(
			wayMatchesExtractTagRules(
				{ id: 4, refs: [], tags: { park_ride: "yes" } },
				rules,
			),
		).toBe(true)
		expect(
			wayMatchesExtractTagRules(
				{ id: 5, refs: [], tags: { building: "yes" } },
				rules,
			),
		).toBe(false)
	})
	...
	test("CONVEYAL_EXTRACT_TAG_FILTERS relation regression", () => {
		...
		expect(
			relationMatchesExtractTagRules(
				{ id: 1, members: [], tags: { type: "restriction" } },
				rules,
			),
		).toBe(true)
		expect(
			relationMatchesExtractTagRules(
				{ id: 2, members: [], tags: { type: "route" } },
				rules,
			),
		).toBe(false)
	})
