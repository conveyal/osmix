...
describe("@osmix/geojson: createOsmFromGeoJSON", () => {
  ...
  it("should error on Polygon with unclosed ring", async () => {
    ...
    await expect(fromGeoJSON(geojson)).rejects.toThrow("Outer ring of Polygon is not closed.");
  });
  ...
  it("should error on unclosed hole ring", async () => {
    ...
    await expect(fromGeoJSON(geojson)).rejects.toThrow("Hole ring of Polygon is not closed.");
  });
