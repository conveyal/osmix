...
export async function fromGeoJSON(
  ...
  for (const update of startCreateOsmFromGeoJSON(osm, await readDataAsGeoJSON(data))) {
    onProgress(update);
  }
  return osm;
}
...
export function* startCreateOsmFromGeoJSON(
  ...
  for (const feature of geojson.features) {
    ...
    if (normalizedFeature.geometry?.type === "Point") {
      ...
      if (coordinates.length < 2) throw Error("Invalid GeoJSON coordinates in LineString.");
      ...
      const outerWayId = createRelation ? nextWayId-- : (featureId ?? nextWayId--);
      osm.ways.addWay({
        id: outerWayId,
        refs: outerNodeRefs,
      ...
      if (createRelation) {
        osm.relations.addRelation({
          ...
          members: [
            ...
            ...holeWayIds.map(
              (id) => ({ type: "way", ref: id, role: "inner" }) as OsmRelationMember,
            ),
          ],
          tags: {
            type: "multipolygon",
  ...
  yield progressEvent("Finished converting GeoJSON to Osmix, building indexes...");
...
function propertiesToTags(properties: Record<string, unknown> | null): OsmTags | undefined {
...
function extractFeatureId(featureId: string | number | undefined): number | undefined {
