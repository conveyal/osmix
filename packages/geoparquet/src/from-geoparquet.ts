...
export class GeoParquetOsmBuilder {
  ...
  processGeoParquetRows(rows: Record<string, unknown>[]) {
    ...
    for (const row of rows) {
      ...
      const geometryData = row[geometryColumn] as Uint8Array | GeoJSON.Geometry | string;
      const tagsData = row[tagsColumn] as Record<string, string | number> | string;
      if (!geometryData) {
        count++;
        continue;
      }
      ...
      const numericId = id !== undefined ? (typeof id === "bigint" ? Number(id) : id) : undefined;
      ...
      if (normalizedGeometry.type === "Point") {
        ...
        const polygonType =
          type ?? (normalizedGeometry.coordinates.length > 1 ? "relation" : "way");
    ...
    if (skippedInvalidGeometries > 0) {
      this.onProgress(
        progressEvent(`Skipped ${skippedInvalidGeometries} features with invalid geometry`, "warn"),
      );
    }
  ...
  private processPoint(geometry: Point, featureId?: number, tags?: OsmTags): number {
  ...
  private processLineString(geometry: LineString, featureId?: number, tags?: OsmTags): number {
    const coordinates = geometry.coordinates;
    if (coordinates.length < 2) throw Error("LineString must have at least 2 coordinates");
    ...
    if (nodeRefs.length < 2) throw Error("LineString must have at least 2 coordinates");
  ...
  private processMultiLineString(geometry: MultiLineString, featureId?: number, tags?: OsmTags) {
    const coordinates = geometry.coordinates;
    if (coordinates.length === 0) throw Error("MultiLineString must have at least one LineString");

    const wayIds: number[] = [];
    for (const line of coordinates) {
      if (line.length < 2) throw Error("LineString must have at least 2 coordinates");
  ...
  private processPolygon(
    ...
    const outerWayId =
      type === "relation" ? this.getNextWayId() : (featureId ?? this.getNextWayId());
    this.osm.ways.addWay({
      id: outerWayId,
      refs: outerNodeRefs,
    ...
    this.osm.relations.addRelation({
      ...
      members: [
        ...
        ...holeWayIds.map((id) => ({ type: "way", ref: id, role: "inner" }) as OsmRelationMember),
      ],
      tags: {
        type: "multipolygon",
        ...tags,
