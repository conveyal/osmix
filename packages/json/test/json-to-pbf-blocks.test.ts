...
describe("json-to-pbf", () => {
  ...
  it("yields grouped blocks", async () => {
    ...
    for await (const block of jsonEntitiesToBlocks(entityGenerator([...nodes, way, relation]))) {
      blocks.push(block);
    }
    ...
    if (!nodeBlock.primitivegroup[0]) throw new Error("nodeBlock.primitivegroup[0] is undefined");
    ...
    if (!wayBlock.primitivegroup[0]) throw new Error("wayBlock.primitivegroup[0] is undefined");
    ...
    if (!relBlock.primitivegroup[0]) throw new Error("relBlock.primitivegroup[0] is undefined");
  ...
  it("enqueues header before entities", async () => {
    ...
    const stream = createOsmJsonReadableStream(header, entityGenerator([...nodes, way]));
