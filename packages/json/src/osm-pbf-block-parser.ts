  ...
  *[Symbol.iterator]() {
    for (const group of this.primitivegroup) {
      ...
      if (group.relations.length > 0) {
        yield group.relations.map((r) => this.parseRelation(r, this.parseOptions));
      }
    }
  }
  ...
  fillInfo(info: OsmPbfInfo): OsmInfoParsed {
    return {
      ...
      user: info.user_sid === undefined ? undefined : this.getString(info.user_sid),
    };
  }
   ...
   */
  parseNode(n: OsmPbfNode, { parseTags = true, includeInfo = false }: ParseOptions = {}): OsmNode {
    const node: OsmNode = {
      id: n.id,
   ...
   */
  parseWay(w: OsmPbfWay, { parseTags = true, includeInfo = false }: ParseOptions = {}): OsmWay {
    let ref = 0;
    const way: OsmWay = {
      id: w.id,
