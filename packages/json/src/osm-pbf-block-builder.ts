...
export class OsmPbfBlockBuilder implements OsmPbfBlock {
  ...
  constructor(
    ...
    this.maxEntitiesPerBlock = blockSettings?.maxEntitiesPerBlock ?? MAX_ENTITIES_PER_BLOCK;
  ...
  addRelation(relation: OsmRelation) {
    ...
    const info = this.includeInfo ? this.addInfo(relation.info ?? {}) : undefined;
    const memids: number[] = [];
  ...
  addDenseNode(node: OsmNode) {
    ...
    if (denseinfo) {
      const { version, timestamp, changeset, uid, user_sid, visible } = denseinfo;
