 ...
 */
export async function getFixtureFile(url: string): Promise<Uint8Array<ArrayBufferLike>> {
...
export function getFixtureFileReadStream(url: string) {
  return Readable.toWeb(createReadStream(getFixturePath(url))) as ReadableStream<Uint8Array>;
}

export function getFixtureFileWriteStream(url: string) {
  return Writable.toWeb(createWriteStream(getFixturePath(url))) as WritableStream<Uint8Array>;
}
