...
export default class StringTable {
  ...
  constructor(opts?: StringTableTransferables) {
    this.bytes = opts?.bytes ? RTA.from(Uint8Array, opts.bytes) : new RTA(Uint8Array);
    this.start = opts?.start ? RTA.from(Uint32Array, opts.start) : new RTA(Uint32Array);
    this.count = opts?.count ? RTA.from(Uint16Array, opts.count) : new RTA(Uint16Array);
  }
