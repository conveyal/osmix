...
export class Relations extends Entities<OsmRelation> {
  ...
  addRelations(
    ...
    for (const relation of relations) {
      ...
      this.memberTypes.pushMany(
        filteredRelation?.members.map((m) => RELATION_MEMBER_TYPES.indexOf(m.type)) ?? memberTypes,
      );
      this.memberRoles.pushMany(
        filteredRelation?.members.map((m) => this.stringTable.add(m.role ?? "")) ?? memberRoles,
      );
    }
    return added;
  }
   ...
   */
  getMembersByIndex(index: number, relationMemberTypes = RELATION_MEMBER_TYPES) {
   ...
   */
  includesMember(index: number, memberRef: number, memberType: OsmEntityType, memberRole?: string) {
    const start = this.memberStart.at(index);
    const count = this.memberCount.at(index);
    for (let i = start; i < start + count; i++) {
      const type = RELATION_MEMBER_TYPES[this.memberTypes.at(i)];
      ...
      if (memberRole !== undefined && this.stringTable.get(this.memberRoles.at(i)) !== memberRole)
        continue;
      return true;
    }
    return false;
